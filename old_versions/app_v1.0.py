...
def generate_prd_document(product_idea: str, research_summary: str | None) -> str:
    ...
    parsed = call_ollama_json(system_prompt, user_prompt, prd_schema, reasoning=True)
...
def generate_engineering_spec_reasoning(
    product_idea: str,
    ...
    return json.dumps({"detailed_steps": reasoning_steps}, indent=2)
...
def generate_engineering_spec(
    ...
    warnings: list[str] = []
    stories_text = user_stories or ""
    user_ac_set = extract_acceptance_criteria(stories_text)
    ...
    previous_contract_block = f"\nPrevious contract attempt to fix/refactor:\n{previous_contract}" if previous_contract else ""
    contract_user_prompt_base = f"""
    Architect's Plan (reasoning steps):
    {reasoning_steps}

    {'Previous attempt QA feedback (what to fix): ' + qa_feedback if qa_feedback else ''}
    {previous_contract_block}
...
def generate_engineering_code_plan(
    product_idea: str,
    user_stories: str | None,
    ...
    if not plan_parsed or not plan_parsed.get("implementation_plan"):
        ...
        return json.dumps({"implementation_plan": "# Failed to generate implementation plan."}, indent=2)
    else:
        logger.info("... Implementation plan generated successfully.")
        return json.dumps(plan_parsed, indent=2)
...
def engineering_spec_reasoning_node(state: AgentState):
    ...
    reasoning_parsed = generate_engineering_spec_reasoning(
        state['product_idea'],
        state.get('user_stories'),
        wireframe_html_clean
    )
    state['reasoning_spec'] = reasoning_parsed
...
def engineering_spec_node(state: AgentState):
    ...
    refactor_directive = state.get("refactor_directive")
    spec_refactor = None
    if isinstance(refactor_directive, dict) and refactor_directive.get("step") == "engineering_spec":
        spec_refactor = refactor_directive
        logger.info("... Refactor directive detected for engineering spec; supplying previous artifact + QA feedback.")

    spec = generate_engineering_spec(
        reasoning_steps=state['reasoning_spec'],
        user_stories=state.get('user_stories'),
        qa_feedback=spec_refactor.get("qa_feedback") if spec_refactor else None,
        previous_contract=spec_refactor.get("previous_artifact") if spec_refactor else None,
    )
...
def engineering_code_reasoning_node(state: AgentState):
    ...
    plan_artifact = generate_engineering_code_plan(
            state['product_idea'],
            state.get('user_stories'),
...
def engineering_node(state: AgentState):
    ...
    refactor_directive = state.get("refactor_directive")
    code_refactor = None
    if isinstance(refactor_directive, dict) and refactor_directive.get("step") == "engineering_code":
        code_refactor = refactor_directive
        logger.info("... Refactor directive detected for engineering code; supplying previous artifact + QA feedback.")

    file_name, code = generate_engineering_code(
        implementation_plan=state['reasoning_code'],
        user_stories=state.get('user_stories'),
        spec_contract=state.get('engineering_spec'),
        qa_feedback=code_refactor.get("qa_feedback") if code_refactor else None,
        previous_code=code_refactor.get("previous_artifact") if code_refactor else None,
    )
...
class ResubmitRequest(BaseModel):
    ...
    mode: Optional[str] = "resubmit"  # "resubmit" (default) or "refactor"
...
async def resubmit_step(request: ResubmitRequest, db: Session = Depends(get_db)):
    ...
    mode = (request.mode or "resubmit").strip().lower()
    if mode not in {"resubmit", "refactor"}:
        raise HTTPException(status_code=400, detail="Invalid resubmission mode.")
    is_refactor = mode == "refactor"
    if is_refactor and request.step not in REFRACTORABLE_STEPS:
        raise HTTPException(status_code=400, detail="Refactor is only available for engineering spec or engineering code.")
    ...
    # Preserve artifacts + QA before wiping when refactoring.
    refactor_directive = None
    if is_refactor:
        if request.step == "engineering_spec":
            previous_artifact = db_task.engineering_spec
            qa_feedback = db_task.engineering_spec_qa
        elif request.step == "engineering_code":
            previous_artifact = db_task.engineering_code
            qa_feedback = db_task.engineering_qa
        else:
            previous_artifact = None
            qa_feedback = None
        if not previous_artifact:
            raise HTTPException(status_code=400, detail="Cannot refactor without a previous artifact to rewrite.")
        if not qa_feedback:
            logger.warning("Refactor requested without QA feedback for task %s step %s.", request.task_id, request.step)
        refactor_directive = {
            "step": request.step,
    ...
    action_label = "refactor" if is_refactor else "resubmit"
    logger.info("Task %s requested %s run for step %s", request.task_id, action_label, request.step)
    ...
    state["pending_approval_content"] = None
    if refactor_directive:
        state["refactor_directive"] = refactor_directive
    ...
    if isinstance(updated_state, dict):
        updated_state.pop("refactor_directive", None)
