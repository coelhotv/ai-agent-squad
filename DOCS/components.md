...
- **Architect + Developer + QA Agents:** `generate_engineering_spec` runs a reasoning prompt then a coding prompt to produce `schemas`/`endpoints`, followed by `run_spec_qa_review`. `generate_engineering_code` consumes the contract to produce the prototype, then `run_engineering_qa_review` tests it for demo-level adherence—QA outputs persist for human review.
