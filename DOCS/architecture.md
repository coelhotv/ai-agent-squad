...
1. **LLM Host (macOS):** Ollama runs outside Docker to keep GPU access to the M2’s Metal cores. The reasoning agents rely on `deepseek-r1:8b-0528-qwen3-q4_K_M` while UX/Engineering/QA use `qwen2.5-coder:7b-instruct-q6_K`; both are available via the same `http://host.docker.internal:11434` endpoint.
...
- **Future Agents:** Engineering, QA, and GTM nodes will continue the serialized path (each pausing for human approvals), keeping the same checkpoint + DB record pattern so everything remains auditable.
